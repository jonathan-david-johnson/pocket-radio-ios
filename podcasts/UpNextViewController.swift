...
class UpNextViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func updateNavBarButtons(animated: Bool = false) {
        // This controller is hosted inside `PlaylistsHostViewController`'s segmented
        // container, where it is added and removed as a child. Writing bar buttons
        // while off-window lands on a navigation item nobody is showing.
        ...
        if rightButtons.count > 1 {
            effectiveNavigationItem.trailingItemGroups = [UIBarButtonItemGroup(barButtonItems: rightButtons, representativeItem: nil)]
        } else {
            effectiveNavigationItem.setRightBarButtonItems(rightButtons, animated: animated)
        }
        effectiveNavigationItem.setLeftBarButton(leftButton, animated: animated)
    }
