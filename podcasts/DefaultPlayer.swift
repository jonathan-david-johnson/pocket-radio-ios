        ...
        if player?.currentItem?.status == .readyToPlay {
            ...
            if assetTrack == nil, let tracks = player?.currentItem?.asset.tracks {
                ...
                for track in tracks {
                    ...
                    // The volume-boost audio mix uses an MTAudioProcessingTap, which
                    // requires a concrete audio asset track. HLS streams don't expose
                    // one (asset.tracks is empty), so attaching the mix breaks audio
                    // playback at non-1x rates — the audio ignores the rate while the
                    // video honors it. Only attach it when we actually found an audio
                    // track.
                    if !skipAudioMix, assetTrack != nil {
                        createAudioMix()
